...
export default function ProfileTab() {
  ...
  const age = displayUser?.date_of_birth ? calculateAge(displayUser.date_of_birth) : null
  const isVerified = Boolean(displayUser?.is_verified)
  const videoUrl = displayUser?.profile_video_url ? String(displayUser.profile_video_url) : null
  const thumbnail = displayUser?.profile_video_thumbnail_url ? String(displayUser.profile_video_thumbnail_url) : null
            ...
            <CardContent className="p-6 items-center">
              <Avatar className="w-24 h-24 mb-4">
                ...
                <AvatarFallback className="bg-purple-500">
                  <Text className="text-white text-2xl font-semibold">{firstName[0]}</Text>
                </AvatarFallback>
              </Avatar>
              <Text className="text-xl font-semibold text-slate-800">{firstName} {lastName}</Text>
              <Text className="text-slate-600 mt-1">{denomination} {age ? `• ${age} years old` : ""}</Text>
